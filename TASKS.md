...
- [x] ~~"Preencher campo `sources` real nas faixas"~~ — **RESOLVIDO 2026-08-29**: schema definido pelo utilizador (`{audio, imagem, data_recolha}`), implementado em todas as 284 faixas. `imagem`/`data_recolha` inferidos de campos já existentes; `audio` deixado `null` retroactivamente (sem sinal fiável no catálogo actual) mas corrigido para faixas futuras (`_aplicar_playlist.py` agora grava-o). Ver STATUS.md.

- [ ] **Mosaico de `STD.INT.10.SYN.002` ("Anos 2010 — Synth II") é de 2007, não dos anos 2010.** `arcade_fire_my_body_is_a_cage` ("My Body Is a Cage", Arcade Fire) é do álbum *Neon Bible* (2007) — já estava anotado no próprio campo `estilo` da faixa como discrepância conhecida, nunca resolvida. Corrigi só os bugs técnicos ao lado (formato do Mosaico, "pessoa" errada — ver STATUS.md 2026-08-29); a década em si fica por decidir: manter (tolerável, é só o Mosaico, não uma das 7 faixas regulares) ou trocar por uma faixa/artista genuinamente dos anos 2010.

- [x] ~~Bug "Gerar com IA" modo Categoria: às vezes gera 6 faixas em vez de 7~~ — **RESOLVIDO 2026-08-29, causa raiz maior do que o reportado**: `montarPromptIA()` e `af_musicasIAComSemente()` tinham o número de faixas pedidas à IA **hardcoded a 6**, correcto só quando o modo activo já fixa exactamente 1 faixa (URL com semente preenchida — 1 fixa + 6 da IA = 7). Nos outros modos isto pedia sempre o número errado: **Categoria e Tema (0 sementes fixas) geravam sempre 6, nunca 7** (não era "às vezes", era sistemático); Semente Múltipla com 2 ou 3 sementes preenchidas pedia 6 a mais, dando 8 ou 9 faixas totais em vez de 7. Corrigido para `nFaixasIA = 7 - faixasFixas.length` em `montarPromptIA()`, `af_montarPromptCorretivoArtistaRepetido()` (prompt de correcção de artista repetido) e `af_musicasIAComSemente()` (usada tanto para detectar artistas repetidos como para o total final). Testado em Chrome real (`_teste_ia_7faixas.js`, scratchpad) nos 5 cenários: Categoria/Tema/URL-sem-semente → 7; URL-com-semente → 6 (inalterado, já estava certo); Semente Múltipla com 2 sementes → 5. Total final confirmado sempre 7 via `af_musicasIAComSemente()`. **Confirmação adicional pedida pelo utilizador (2026-08-29)**: faltava testar explicitamente o modo `playlist-youtube` (o 5º modo, não coberto na primeira ronda de testes) e o prompt correctivo de artista repetido (`af_montarPromptCorretivoArtistaRepetido`, o 2º sítio com o mesmo bug) — ambos testados agora (`_teste_ia_playlist_youtube.js`, scratchpad): `playlist-youtube` pede 7 correctamente, total final combinado 7, prompt correctivo também usa 7. **Todos os modos afectados confirmados cobertos, sem excepção.**

- [x] **`fastForward()` testado com `DURACAO_TRECHO_B` (2026-08-29).** Teste real em Chrome (`_teste_fastforward.js`, scratchpad) numa faixa Standard real (`dec50_01`, Trecho B confirmado 18-30s = 12s tanto no JSON como no ficheiro `.mp3` real via `afinfo`): `fastForward()` funciona correctamente — avança `trechoAtual` 1→2, carrega e toca `dec50_01_b.mp3`, sem erros. **Achado**: `DURACAO_TRECHO_B` (=10, linha ~823) está declarada mas **nunca é usada em lado nenhum do código** — `fastForward()` usa sempre `JANELA_TRECHO` (=12) para o Trecho B também, o que está correcto porque os ficheiros reais de Trecho B duram ~12s (`gerar_faixa.py`: `ITUNES_TRECHO_B=(18,30)`), não 10s. Se `fastForward()` alguma vez passasse a usar `DURACAO_TRECHO_B`, cortaria os últimos ~2s de todos os Trechos B do jogo — seria uma regressão, não uma correcção. **Consequência a decidir**: o texto `duracao_estimada.nota` das 16 playlists criadas nesta sessão (2026-08-27/29, via `_aplicar_playlist.py`) diz "10s TrechoB", que está desactualizado/errado face a este teste — e a própria fórmula `PURA_SEG = 7×22s` tem uma inconsistência aritmética interna (12+8+10=30, não 22) que não investiguei a fundo. Não corrigido aqui — fora do âmbito do pedido original ("testar", não "corrigir 16 playlists"), fica reportado para decisão.

- [ ] **Auditoria "membro individual vs banda" — segunda ronda (2026-08-29), só reporte.** `af_verificarConsistenciaArtistaBanda()` (o verificador já existente no Studio) corrida de novo: **0 faixas sinalizadas** (lista curada de 16 bandas + 4 músicos "nunca a solo"). Alargada manualmente a ~40 membros de banda conhecidos (Beatles, Stones, Queen, Zeppelin, Floyd, Nirvana, Metallica, GNR, etc.) contra os 277 ids com um só `artista`, fora de Solistas/Retrato Sonoro (onde a pessoa é sempre a resposta certa por desenho): **nenhum caso novo real encontrado** — todos os que apareceram (Jimi Hendrix, Muddy Waters, Frankie Goes to Hollywood, Fleetwood Mac, Ringo Starr em `beatles_08`) são créditos correctos/deliberados (nomes a solo legítimos, nomes de banda a solo correctos, ou excepção contextual válida de Tributo). **Uma tensão genuína encontrada, não corrigida**: `bat_02` ("We Will Rock You") é partilhada por duas playlists — "Solo de Bateria" (quer a pessoa, Roger Taylor, já correcto) e "Anos 70 — Rock" (`STD.INT.70.ROC.001`, onde um trivia geral tipicamente esperaria a banda, "Queen"). O campo `artista` é único por faixa, não pode satisfazer os dois contextos em simultâneo — fica para decisão do utilizador (aceitar como está, ou remover `bat_02` de uma das duas playlists).

- [x] ~~`01_a.mp3`/`01_b.mp3`~~ — **DECIDIDO 2026-08-29: manter como estão.** Não são órfãos de conteúdo — servem o fallback offline hardcoded `COLLECTIONS.ANOS80` em `musicbox.html` (usado só se `catalogo.json`/`playlists.json` falharem a carregar). Utilizador confirmou que são infra-estrutura do fallback, não um gap de conteúdo por resolver. Nenhuma alteração feita.
