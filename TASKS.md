...
- [ ] **"Preencher campo `sources` real nas faixas" — bloqueado, precisa de definição (2026-08-29).** Procurei em todo o repositório (`.py`, `.html`, `.md`, `catalogo.json`) e o campo `sources`/`source` **não existe em lado nenhum** — nenhuma faixa o tem, nenhum código o lê ou escreve, nenhuma documentação o define. Não há um schema estabelecido para eu seguir. Antes de inventar um formato (que campos, string vs array, uma fonte por faixa ou várias — áudio? imagem? letra/verificação de facto?), preciso de saber o que se pretende que este campo contenha. Não implementado — fica para confirmação do utilizador antes de avançar.
...
- [x] ~~Bug "Gerar com IA" modo Categoria: às vezes gera 6 faixas em vez de 7~~ — **RESOLVIDO 2026-08-29, causa raiz maior do que o reportado**: `montarPromptIA()` e `af_musicasIAComSemente()` tinham o número de faixas pedidas à IA **hardcoded a 6**, correcto só quando o modo activo já fixa exactamente 1 faixa (URL com semente preenchida — 1 fixa + 6 da IA = 7). Nos outros modos isto pedia sempre o número errado: **Categoria e Tema (0 sementes fixas) geravam sempre 6, nunca 7** (não era "às vezes", era sistemático); Semente Múltipla com 2 ou 3 sementes preenchidas pedia 6 a mais, dando 8 ou 9 faixas totais em vez de 7. Corrigido para `nFaixasIA = 7 - faixasFixas.length` em `montarPromptIA()`, `af_montarPromptCorretivoArtistaRepetido()` (prompt de correcção de artista repetido) e `af_musicasIAComSemente()` (usada tanto para detectar artistas repetidos como para o total final). Testado em Chrome real (`_teste_ia_7faixas.js`, scratchpad) nos 5 cenários: Categoria/Tema/URL-sem-semente → 7; URL-com-semente → 6 (inalterado, já estava certo); Semente Múltipla com 2 sementes → 5. Total final confirmado sempre 7 via `af_musicasIAComSemente()`.
