...
- [ ] **`ptbat_01` (UHF, "Cavalos de Corrida", 1980) e `ptbat_05` (RAMP, "Last Child")** — playlist "Bateria Portuguesa" (`STD.PT.ALL.BAT.001`), agora no universo "Solistas". Das 7 faixas, 5 já têm `artista` corrigido para o nome do baterista real (`ptbat_02` Kalú/Xutos & Pontapés, `ptbat_03` Tóli César Machado/GNR, `ptbat_04` Kinörm/Ornatos Violeta, `ptbat_06` Hélio Morais/Linda Martini, `ptbat_07` Salvador Seabra/Capitão Fausto — confirmados via Wikipedia PT, texto sourced). Estas duas ficaram de fora de propósito — `artista` mantém-se a banda (`UHF`/`RAMP`), com `imagem_risco_ambiguidade: true` e `nota_curadoria` a explicar porquê:
  - **UHF**: pelo menos 6 bateristas diferentes ao longo da história (Alfredo Antunes → Américo Manuel → Zé Carvalho → Manuel Hippo → Rui Beat Velez → Luís Espírito Santo → ... → Ivan Cristiano desde 1999) — a Wikipedia não dá datas suficientes para confirmar quem tocava especificamente em 1980. Precisa de fonte primária (arquivo/discografia do álbum/single, não Wikipedia).
  - **RAMP**: confirmado que a banda certa é a portuguesa de thrash metal (não o DJ homónimo que a imagem actual mostra — essa parte da desconfiança original estava certa). Mas a Wikipedia só lista "ex-integrantes" sem instrumento nem datas por pessoa — impossível identificar o baterista de "Last Child" sem saber sequer de que álbum é.
  - Imagem de nenhuma das duas foi corrigida (continua a foto da banda/DJ errado) — sem fonte fiável disponível até se resolver a identidade.
