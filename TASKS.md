...
- [ ] **`ptbat_05` (RAMP, "Last Child")** — Wikipedia PT confirma que a banda certa é a portuguesa de thrash metal R.A.M.P. (não o DJ homónimo que a imagem actual mostra), mas só lista "ex-integrantes" sem instrumento nem datas por pessoa — impossível identificar o baterista de "Last Child" sem saber sequer de que álbum é. `artista`/`banda` mantidos como estavam (`RAMP`), `imagem_risco_ambiguidade: true` e `nota_curadoria` continuam. Precisa de fonte primária (discografia/encarte de álbum), não Wikipedia.
